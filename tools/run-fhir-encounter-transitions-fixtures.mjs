...
const fixtureIndexPath = process.argv[2] || path.join(root, 'fixtures', 'fhir-encounter-transitions', 'index.json');
...
function evaluateCase(casePath, expectedBundleId, decisionSpace) {
  ...
  if (d.type !== 'FHIREncounterTransitionIntent') {
    return { status: 'fail', code: 'CIC_CANONICAL_VALUE_INVALID', details: 'type must be FHIREncounterTransitionIntent' };
  }
  if (typeof d.intent_class !== 'string' || !decisionSpace.intent_resource_map[d.intent_class]) {
    return { status: 'fail', code: 'ESCALATE_OUT_OF_SCOPE_INTENT', details: 'intent is outside encounter transition bounded domain' };
  }
  ...
  if (d.intent_class === 'fhir.encounter.discharge.v1' && !['discharged', 'completed'].includes(d.encounterStatus)) {
    return { status: 'fail', code: 'CIC_CANONICAL_VALUE_INVALID', details: 'encounterStatus must be discharged|completed for discharge intent' };
  }
  if (d.intent_class === 'fhir.encounter.transfer.v1' && d.encounterStatus !== 'in-progress') {
    return { status: 'fail', code: 'CIC_CANONICAL_VALUE_INVALID', details: 'encounterStatus must be in-progress for transfer intent' };
  }

  return { status: 'pass', code: 'PASS', details: 'CIC and PSC checks passed for FHIR encounter transitions demo pack' };
}

function main() {
  const fixtureIndex = loadJson(fixtureIndexPath);
  const packPath = path.join(root, fixtureIndex.pack_path || 'verticals/fhir-encounter-transitions-demo-pack');
