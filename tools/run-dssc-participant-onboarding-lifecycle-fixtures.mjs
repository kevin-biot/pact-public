...
const fixtureIndexPath = process.argv[2] || path.join(root, 'fixtures', 'dssc-participant-onboarding-lifecycle', 'index.json');
...
function transitionAllowed(currentState, nextState) {
  if (nextState === 'revoked') return true;
  const matrix = {
  ...
  return (matrix[currentState] || []).includes(nextState);
}
...
function evaluateCase(casePath, expectedBundleId, decisionSpace) {
  ...
  if (d.type !== 'DSSCParticipantLifecycleIntent') {
    return { status: 'fail', code: 'CIC_CANONICAL_VALUE_INVALID', details: 'type must be DSSCParticipantLifecycleIntent' };
  }
  if (typeof d.intent_class !== 'string' || !decisionSpace.intent_resource_map[d.intent_class]) {
    return { status: 'fail', code: 'ESCALATE_OUT_OF_SCOPE_INTENT', details: 'intent is outside participant lifecycle bounded domain' };
  }
  ...
  for (const field of ['participant_id', 'current_state', 'next_state']) {
    if (!reqStr(field, d[field])) {
      return { status: 'fail', code: 'CIC_REQUIRED_FIELD_MISSING', details: field + ' is required' };
    }
  }

  if (d.current_state === 'revoked' && d.execution_attempt === true) {
    return { status: 'fail', code: 'DENY_PARTICIPANT_REVOKED', details: 'revoked participant execution attempt denied' };
  }

  if (d.next_state === 'admitted' && d.trust_proof_valid !== true) {
    return { status: 'fail', code: 'DENY_TRUST_PROOF_MISSING', details: 'admission requires trust proof' };
  }

  if (!transitionAllowed(d.current_state, d.next_state)) {
    return { status: 'fail', code: 'DENY_STATE_TRANSITION_INVALID', details: 'lifecycle transition is not allowed' };
  }

  return { status: 'pass', code: 'ALLOW_IN_SCOPE', details: 'participant lifecycle checks passed' };
}

function main() {
  const fixtureIndex = loadJson(fixtureIndexPath);
  const packPath = path.join(root, fixtureIndex.pack_path || 'verticals/dssc-participant-onboarding-lifecycle-demo-pack');
